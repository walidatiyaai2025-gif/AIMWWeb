...
| `/admin/roles-permissions` | Create custom role / assign it to an application user | `Settings.Manage` → `ApplicationRoleAdministrationService` → `ApplicationRoleStore` (`Security.CustomRoles`) + authorization audit; assignment → `ApplicationUserAdministrationService` → `AuthUsers` + session revocation + account audit → refreshed UI | BROWSER ACCEPTANCE IN REVIEW | `RolesPermissionsUxTests` | Current #183 slice proves least-privilege role persistence, reserved-permission exclusion, `Role.Created` audit, role assignment persistence, `Account role changed.` session revocation, `User.Updated` audit and reconciled role display |
...
- The current Roles & Permissions slice drives `/admin/roles-permissions` through the visible UI. It requires a custom role with only `Content.View` and `Content.Edit` to persist in `Security.CustomRoles`, proves reserved `Users.Manage`/`Settings.Manage` are absent, requires `Role.Created` audit, then assigns the role to a real application account and verifies persisted role change, session revocation, `User.Updated` audit and refreshed role display.
...
1. Land exact-head green browser acceptance for `/admin/roles-permissions`, then continue the remaining user-facing scan across Reports/Exports, Settings, synchronization/operations and other visible surfaces.
