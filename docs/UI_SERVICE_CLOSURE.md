...
| `/execution-center`, `/module/execution` | Monitor runtime work | real tracker / approved-change worker → `ExecutionCenterService`; no simulator | BROWSER VERIFIED | `ExecutionCenterTests`; `ExecutionCenterExternalExecutionTests`; `ExecutionAuthenticityUxTests` | PR #192; none |
...
- The current content-editor mutation acceptance drives both a post and a page through the real editor, proves Basic-authenticated WordPress update requests with the user-entered values, requires the normal five-endpoint synchronization sweep, then reloads the editor and requires the values to come back from the remote WordPress result. It must not be promoted to BROWSER VERIFIED until its exact PR head is green and merged.
...
2. After the current content-editor mutation acceptance is green and merged, prioritize approval execution/reconciliation and then session revocation/browser evidence where dependency-safe.
