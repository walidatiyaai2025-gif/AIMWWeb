...
| `/settings` | View application database provider/readiness | `IConfiguration` runtime keys `Database:Provider` + `Database:SetupComplete`, matching `DatabaseSetupService` provider contract; no connection strings or secrets exposed | BROWSER ACCEPTANCE IN REVIEW | `SettingsDatabaseProviderHonestyContractTests`; `SettingsDatabaseProviderUxTests` | Current #183 slice removes the hard-coded SQLite/Active/local-portable claim and requires the configured provider and setup state to render from runtime configuration |
...
- The current Settings slice removes an inaccurate fixed SQLite/Active/local-storage presentation. Settings now reports only the provider and setup state present in runtime configuration and explicitly avoids displaying connection strings or secrets.
...
1. Land exact-head green Settings database-provider honesty evidence, then continue the remaining user-facing scan across synchronization/operations and other visible surfaces.
