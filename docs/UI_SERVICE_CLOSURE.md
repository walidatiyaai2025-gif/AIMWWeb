...
- The repository UX Regression Gate contains dedicated `seo-audit` and `comments-moderation` shards, with contract tests requiring both shards to remain present.
...
- Reconstruct remaining user-visible actions from current `main` after PR #178 is integrated.
- Prefer unverified WordPress mutations, dead/no-op controls, and UI handlers that stop before a real service or remote API boundary.
