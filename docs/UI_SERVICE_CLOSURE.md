...
| `/module/sync` | Run synchronization and recover persistent history after a failed sync | owned site → `ReviewConflictsAsync` / `SynchronizeAsync` → local mirror + `GetHistoryAsync`; secondary history-read failures are surfaced without replacing the primary sync error | CONTRACT VERIFIED | `GlobalSynchronizationWorkspaceHonestyContractTests` | Current #183 slice removes the swallowed history-refresh exception; browser failure injection is not yet claimed |
...
- The current synchronization slice preserves the original synchronization failure and explicitly reports a secondary persistent-history refresh failure instead of swallowing it or leaving stale history presented as freshly reconciled.
...
1. Land exact-head green synchronization error-state honesty evidence, then continue the remaining user-facing scan across Site Operations, maintenance/reliability, logs/health, and other visible surfaces.
