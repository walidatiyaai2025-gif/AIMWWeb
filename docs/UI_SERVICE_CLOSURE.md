...
| `/reports`, `/module/reports` | Load live reports / export Sites CSV | owner-scoped `SiteWebService` + approval/automation/planner services → rendered application data → `aiwmReports.downloadCsv` browser Blob download | BROWSER VERIFIED | `ReportsExportsUxTests` | PR #206; none |
| `/settings` | View application database provider/readiness | `IConfiguration` runtime keys `Database:Provider` + `Database:SetupComplete`, matching `DatabaseSetupService` provider contract; no connection strings or secrets exposed | BROWSER VERIFIED | `SettingsDatabaseProviderHonestyContractTests`; `SettingsDatabaseProviderUxTests` | PR #209; none |
| `/module/sync` | Run synchronization and recover persistent history after a failed sync | owned site → `ReviewConflictsAsync` / `SynchronizeAsync` → local mirror + `GetHistoryAsync`; secondary history-read failures are surfaced without replacing the primary sync error | CONTRACT VERIFIED | `GlobalSynchronizationWorkspaceHonestyContractTests` | PR #210; browser failure injection is not yet claimed |
| `/system-health` | Inspect configured database and platform health | real `SystemHealthWebService` → configured provider/storage/log diagnostics | CONTRACT VERIFIED | `SystemHealthDatabaseProviderHonestyContractTests` | PR #212 merged exact-head green as `2ddb01deda1c852a1c6859e6d7fc6101f771be27`; authorization boundary remains under review |
| `/logs`, `/module/logs` | Inspect and copy live server log diagnostics | `Settings.Manage` route authorization → real `LogReaderService` files/lines | CONTRACT VERIFIED | `OperationalDiagnosticsAuthorizationContractTests` | Current #183 slice adds the missing privileged authorization boundary before live log access |
...
- PR #206 browser-verifies owner-scoped reports and real CSV browser download.
- PR #209 reports the real configured database provider and setup state without exposing connection strings or secrets.
...
- PR #212 makes System Health database diagnostics provider-aware on the configured EF connection.
- The current slice closes a privileged-data gap: live server log contents are now behind the established `Settings.Manage` policy while retaining their real `LogReaderService` destination.
...
1. Finish operational diagnostics authorization evidence, then continue the remaining user-facing scan across Site Operations maintenance/reliability and other visible surfaces.
