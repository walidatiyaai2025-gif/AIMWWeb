...
class ContentExplorerExecutionLinkTerminalityTest extends TestCase
{
    use RefreshDatabase;

    private const OPERATION_ID = 'AIMW-AUTO-22BC08CEF1';

    public function test_exact_canonical_operation_matches_content_explorer_execution_visible_control(): void
    {
        ...
        $this->assertSame('/sites/{Id:guid}/explorer', $operation['route_screen']);
    ...
    public function test_destination_is_the_existing_guarded_execution_workspace(): void
    {
        ...
        $this->assertSame('operations.manage,execution.view', $route->defaults['workspace_permissions'] ?? null);
    ...
    public function test_execution_workspace_enforces_permission_and_tenant_isolation(): void
    {
        ...
        $this->membership($alphaUser, 'alpha', ['tenant.view', 'sites.view', 'execution.view', 'operations.manage']);

        $betaUser = User::factory()->create();
        $this->membership($betaUser, 'beta', ['tenant.view', 'sites.view', 'execution.view', 'operations.manage']);

        $restrictedUser = User::factory()->create();
        $this->membership($restrictedUser, 'restricted', ['tenant.view', 'sites.view', 'execution.view']);
    ...
    public function test_frontend_binding_is_tenant_derived_permission_aware_and_operation_specific(): void
    {
        ...
        $controlSource = (string) file_get_contents(resource_path('js/content-explorer-execution-link-control.tsx'));

        $this->assertStringContainsString("route.key === 'explorer'", $appSource);
        $this->assertStringContainsString('ContentExplorerExecutionLinkControl context={context}', $appSource);
        ...
        $this->assertStringContainsString("route.key === 'explorer'", (string) file_get_contents(resource_path('js/app.tsx')));
        $this->assertStringContainsString("route.key === 'explorer'", $appSource);
        $this->assertStringContainsString("hasPermission(context, 'operations.manage')", $controlSource);
        $this->assertStringContainsString('hasPermission(context, explorerRoute.permission)', $controlSource);
    ...
    private function membership(User $user, string $slug, array $permissions): void
    {
        ...
        $role = Role::query()->create([
            'name' => "content-explorer-execution-{$slug}-{$user->id}",
        ]);
