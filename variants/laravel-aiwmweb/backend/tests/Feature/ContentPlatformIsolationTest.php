...
final class FakeContentDriver implements ContentRemoteDriver
{
    ...
    public function get(int $siteId,string $resource,int $remoteId,array $query=[]): array { return ['id'=>$remoteId,'title'=>['rendered'=>'remote changed'],'modified_gmt'=>'2026-08-27T13:00:00Z','version'=>'v2']; }
    public function mutate(int $siteId,string $resource,?int $remoteId,string $action,array $payload=[]): array { $this->mutations++; return ['id'=>$remoteId ?? 99]+$payload; }
    public function upload(int $siteId,string $path,string $name,string $mimeType,array $metadata=[]): array { return ['id'=>100,'source_url'=>'https://example.test/'.$name]; }
    public function semantic(int $siteId,string $operation,array $payload=[]): array { return []; }
}
