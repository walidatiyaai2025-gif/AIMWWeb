...
final class RoutesOpenMyAccountSecurityTest extends TestCase
{
    ...
    {
        $user = User::factory()->create(['name' => 'Alpha Account User', 'email' => 'alpha-account@example.test']);
        ...
        $this->actingAs($user)->get('/tenants/alpha/account/profile')->assertOk()->assertSee('id="app"', false);
    }

    public function test_public_access_denied_and_json_denials_never_expose_the_profile_control(): void
    {
        $this->get('/access-denied')->assertOk()->assertSee('Access denied')->assertDontSee(self::OPERATION_ID)->assertDontSee('Open my account');
        $user = User::factory()->create();
        $this->membership($user, 'alpha', ['tenant.view'], 'JSON Denial Role');
        $this->actingAs($user)->getJson('/tenants/alpha/account/billing?tenant=beta&user_id=999999')
            ->assertForbidden()->assertHeader('content-type', 'application/json')->assertDontSee(self::OPERATION_ID)->assertDontSee('Open my account')->assertDontSee('/tenants/alpha/account/profile');
    }

    public function test_guest_missing_tenant_view_and_foreign_tenant_attempts_fail_closed_without_the_control(): void
    {
        $this->get('/tenants/alpha/account/billing')->assertRedirect('/login')->assertDontSee(self::OPERATION_ID);
        $limited = User::factory()->create();
        $this->membership($limited, 'limited', [], 'Limited Role');
        $this->actingAs($limited)->get('/tenants/limited/account/billing')->assertForbidden()->assertDontSee(self::OPERATION_ID)->assertDontSee('Open my account');
        ...
        $this->actingAs($alpha)->get('/tenants/beta/account/billing')->assertNotFound()->assertDontSee(self::OPERATION_ID);
    ...
    public function test_profile_destination_and_authoritative_read_are_explicit_guarded_get_only_and_direct_id_free(): void
    {
        ...
        $this->assertSame(CanonicalWorkspaceRouteController::class.'@show', ltrim($profile->getActionName(), '\\'));
        ...
        $this->assertSame(RouteApiAdapterController::class.'@accountProfile', ltrim($api->getActionName(), '\\'));
    ...
    {
        $alpha = User::factory()->create(['name' => 'Authoritative Alpha', 'email' => 'authoritative-alpha@example.test', 'password' => bcrypt('alpha-secret-password')]);
        $alphaMembership = $this->membership($alpha, 'alpha', ['tenant.view'], 'Alpha Profile');
        $beta = User::factory()->create(['name' => 'Foreign Beta', 'email' => 'foreign-beta@example.test', 'password' => bcrypt('beta-secret-password')]);
        $betaMembership = $this->membership($beta, 'beta', ['tenant.view'], 'Beta Profile');
        $before = ['users' => User::query()->count(), 'memberships' => TenantMembership::query()->withoutGlobalScopes()->count(), 'roles' => Role::query()->withoutGlobalScopes()->count()];
        $path = '/tenants/alpha/route-api/account-profile?user_id='.$beta->id.'&membership_id='.$betaMembership->id.'&tenant=beta&email='.$beta->email;
        ...
        $after = ['users' => User::query()->count(), 'memberships' => TenantMembership::query()->withoutGlobalScopes()->count(), 'roles' => Role::query()->withoutGlobalScopes()->count()];
