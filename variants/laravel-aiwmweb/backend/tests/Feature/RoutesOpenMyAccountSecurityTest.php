...
final class RoutesOpenMyAccountSecurityTest extends TestCase
{
    ...
    public function test_authenticated_denial_with_profile_access_renders_only_the_active_tenant_account_control(): void
    {
        ...
        $denied = $this->actingAs($user)
            ->get('/tenants/alpha/account/billing?tenant=beta&user_id=999999')
            ->assertForbidden()
        ...
        $this->actingAs($user)
            ...
            ->assertJsonPath('data.0.name', 'Alpha Account User')
            ->assertJsonPath('data.0.email', 'alpha-account@example.test')
    ...
    public function test_profile_destination_and_authoritative_read_are_explicit_guarded_and_direct_id_free(): void
    {
