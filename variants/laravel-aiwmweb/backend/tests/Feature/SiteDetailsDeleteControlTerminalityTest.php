...
class SiteDetailsDeleteControlTerminalityTest extends TestCase
{
    ...
    public function test_guest_and_missing_permission_fail_closed_without_persistence_change(): void
    {
        ...
        $this->actingAs($owner)
            ->deleteJson('/api/tenants/alpha-denied/sites/'.$site->id)
            ->assertForbidden();
        $this->assertDatabaseHas('sites', ['id' => $site->id, 'tenant_id' => $membership->tenant_id]);
    }

    public function test_authorized_delete_is_tenant_scoped_persisted_authoritatively_verified_and_replay_safe(): void
    {
        ...
        $this->actingAs($user)
            ->deleteJson('/api/tenants/alpha-delete/sites/'.$site->id)
            ->assertNotFound();
    }

    public function test_route_tenant_mismatch_foreign_direct_id_and_invalid_id_fail_closed(): void
    {
        $user = User::factory()->create();
        ...
        $this->actingAs($user)
            ->deleteJson('/api/tenants/beta-owner/sites/'.$alphaSite->id)
            ->assertNotFound();
        $this->actingAs($user)
            ->deleteJson('/api/tenants/alpha-owner/sites/'.$betaSite->id)
            ->assertNotFound();
        $this->actingAs($user)
            ->deleteJson('/api/tenants/alpha-owner/sites/not-a-number')
            ->assertNotFound();
        $this->actingAs($user)
            ->deleteJson('/api/tenants/alpha-owner/sites/0')
            ->assertNotFound();

        $this->assertDatabaseHas('sites', ['id' => $alphaSite->id, 'tenant_id' => $alpha->tenant_id]);
        $this->assertDatabaseHas('sites', ['id' => $betaSite->id, 'tenant_id' => $beta->tenant_id]);
    }
    ...
    public function test_queued_or_running_execution_blocks_delete_and_preserves_site(): void
    {
        foreach (['queued', 'running'] as $status) {
            ...
            $this->actingAs($user)
                ->deleteJson('/api/tenants/'.$membership->tenant->slug.'/sites/'.$site->id)
                ->assertConflict();
            $this->assertDatabaseHas('sites', ['id' => $site->id, 'tenant_id' => $membership->tenant_id]);
        }
    }
    ...
    public function test_completed_execution_does_not_create_false_active_blocker(): void
    {
        ...
        $this->actingAs($user)
            ->deleteJson('/api/tenants/execution-completed/sites/'.$site->id)
            ->assertNoContent();
    ...
    private function site(TenantMembership $membership, string $name): Site
    {
        ...
        $site = Site::query()->create([
            'name' => $name,
            'url' => 'https://example.test/'.strtolower(str_replace(' ', '-', $name)),
            'status' => 'active',
        ]);
        $context->forget();
    ...
    private function executionFixture(TenantMembership $membership, Site $site, User $user, string $status): void
    {
        ...
        $now = now();
        $syncedContentId = DB::table('synced_contents')->insertGetId([
            'tenant_id' => $tenantId,
            'site_id' => $site->id,
            'resource_type' => 'post',
            'remote_id' => random_int(1000, 999999),
            'slug' => 'delete-guard-'.Str::lower(Str::random(8)),
            'created_at' => $now,
            'updated_at' => $now,
        ]);
        ...
        $findingId = DB::table('seo_findings')->insertGetId([
            ...
            'synced_content_id' => $syncedContentId,
            'code' => 'delete-guard',
            'severity' => 'warning',
        ...
        DB::table('executions')->insert([
            ...
            'started_at' => $status === 'running' || $status === 'completed' ? $now : null,
