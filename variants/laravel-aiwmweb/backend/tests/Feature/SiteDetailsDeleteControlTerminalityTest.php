...
class SiteDetailsDeleteControlTerminalityTest extends TestCase
{
    ...
    public function test_authorized_delete_is_tenant_scoped_persisted_and_authoritatively_verified(): void
    {
        ...
        $this->csrfDeleteAs($user, '/api/tenants/alpha/sites/'.$site->id)->assertNoContent();
        $this->assertDatabaseMissing('sites', ['id' => $site->id, 'tenant_id' => $alpha->tenant_id]);
        $this->csrfDeleteAs($user, '/api/tenants/alpha/sites/'.$site->id)->assertNotFound();
    }
    ...
    public function test_permission_foreign_id_and_active_execution_fail_closed(): void
    {
        ...
        $this->csrfDeleteAs($limited, '/api/tenants/limited/sites/'.$limitedSite->id)->assertForbidden();
        ...
        $this->csrfDeleteAs($owner, '/api/tenants/alpha/sites/'.$betaSite->id)->assertNotFound();
        $this->csrfDeleteAs($owner, '/api/tenants/alpha/sites/not-a-number')->assertNotFound();
        $this->csrfDeleteAs($owner, '/api/tenants/alpha/sites/0')->assertNotFound();
        ...
        $this->csrfDeleteAs($owner, '/api/tenants/alpha/sites/'.$alphaSite->id)->assertConflict();
        $this->assertDatabaseHas('sites', ['id' => $alphaSite->id]);
    }

    private function csrfDeleteAs(User $user, string $url): TestResponse
    {
        $this->actingAs($user)->withSession([]);
        $token = (string) session()->token();
        return $this->withHeader('X-CSRF-TOKEN', $token)->deleteJson($url);
    }
