...
class SiteOperationsMaintenanceRouteTerminalityTest extends TestCase
{
    use RefreshDatabase;

    private const OPERATION_ID = 'AIMW-AI-959B247B1D';
    ...
    {
        $route = Route::getRoutes()->match(Request::create('/tenants/alpha/site-operations/maintenance', 'GET'));

        $this->assertSame(SiteOperationsMaintenanceReadController::class, ltrim($route->getActionName(), '\\'));
        $this->assertSame('canonical.workspace.site-operations-maintenance', $route->getName());
    ...
    public function test_authorized_route_renders_real_tenant_scoped_storage_and_preview(): void
    {
        ...
        $response
            ->assertOk()
            ->assertViewIs('site-operations-maintenance')
    ...
    public function test_guest_missing_permission_and_cross_tenant_access_fail_closed(): void
    {
        ...
        $this->actingAs($limited)->get('/tenants/limited/site-operations/maintenance')->assertForbidden();
        ...
        $this->actingAs($alpha)->get('/tenants/beta/site-operations/maintenance')->assertNotFound();
    }
