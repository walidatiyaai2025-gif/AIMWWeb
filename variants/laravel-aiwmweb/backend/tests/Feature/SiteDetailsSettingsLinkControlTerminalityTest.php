...
class SiteDetailsSettingsLinkControlTerminalityTest extends TestCase
{
    use RefreshDatabase;

    private const OPERATION_ID = 'AIMW-AI-2B31C6BDAF';

    public function test_canonical_operation_is_the_pending_site_details_interpolated_navigation_control(): void
    {
        ...
        $this->assertSame('@($ -> @($', $operation['visible_control']);
        $this->assertFalse((bool) $operation['mutation']);
        $this->assertTrue((bool) $operation['tenant_owned']);
    }

    public function test_authorized_user_can_open_site_details_then_boot_settings_with_the_same_tenant_scoped_site(): void
    {
        ...
        $this->actingAs($user)
            ->get('/tenants/alpha/settings?site='.$site->id)
            ->assertOk()
            ->assertSee('id="app"', false);
    ...
    public function test_source_route_and_selected_site_context_fail_closed_for_guest_permission_and_foreign_tenant_ids(): void
    {
        ...
        $this->get('/tenants/alpha/settings?site='.$alphaSite->id)->assertRedirect('/login');
    ...
    private function membership(User $user, string $slug, array $permissions): TenantMembership
    {
        ...
        $role = Role::query()->create(['name' => "site-settings-nav-{$slug}-{$user->id}"]);
