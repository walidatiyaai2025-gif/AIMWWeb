...
final class ContentTransferJob extends TenantAwareJob
{
    ...
    public function uniqueId(): string { return "tenant:{$this->tenantId}:content-transfer:{$this->transferId}"; }
    ...
    public function handle(ContentPlatformService $service): void
    {
        ...
        try {
            ...
            if (! $transfer->storage_path || ! Storage::disk('local')->exists($transfer->storage_path)) throw new RuntimeException('Import source is unavailable.');
            $payload = json_decode(Storage::disk('local')->get($transfer->storage_path), true, 512, JSON_THROW_ON_ERROR);
            if (($payload['version'] ?? null) !== 1 || ! is_array($payload['content'] ?? null)) throw new RuntimeException('Unsupported or invalid content import format.');
            $rows = $payload['content']; $total=max(count($rows),1); $done=0;
            foreach ($rows as $row) {
                if (! in_array($row['type'] ?? null, ['post','page'], true)) continue;
                ...
                $done++; $transfer->update(['progress'=>min(99, 5 + (int) floor(($done/$total)*90))]);
            }
