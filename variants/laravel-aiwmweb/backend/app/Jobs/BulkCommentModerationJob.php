...
final class BulkCommentModerationJob extends TenantAwareJob
{
    ...
    public function __construct(int $tenantId, public readonly int $siteId, public readonly array $commentIds, public readonly string $action)
    {
    ...
    {
        return "tenant:{$this->tenantId}:site:{$this->siteId}:comments:".hash('sha256', json_encode([$this->commentIds, $this->action]));
    }
    ...
    public function handle(ContentPlatformService $service): void
    {
        ...
        foreach ($comments as $comment) {
            $service->mutateComment($this->siteId, $comment->remote_id, $this->action);
        }
    }
}
