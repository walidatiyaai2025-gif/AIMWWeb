...
final class ContentPlatformService
{
    ...
    public function content(int $siteId, string $type, array $filters = []): LengthAwarePaginator
    {
        ...
        if ($search = trim((string) ($filters['search'] ?? ''))) {
            $q->where(fn ($x) => $x->where('title', 'like', "%{$search}%")->orWhere('slug', 'like', "%{$search}%")->orWhere('body', 'like', "%{$search}%"));
        }
    ...
    public function sync(int $siteId, bool $full = false): array
    {
        ...
        foreach ($resources as $index => $resource) {
            ...
            try {
                ...
                $changed = 0;
                $maxModified = $state->last_remote_modified_at;
                foreach ($rows as $row) {
                    if (! is_array($row)) {
                        continue;
                    }
    ...
    public function mutateContent(int $siteId, string $type, ?int $remoteId, string $action, array $payload, array $expected = []): array
    {
        ...
        if ($item && ! in_array($action, ['create'], true)) {
            $this->guardConflict($siteId, $resource, $item, $expected);
        }
        if ($item && in_array($action, ['update', 'publish', 'draft', 'pending', 'schedule', 'trash', 'delete', 'restore'], true)) {
            $this->snapshot($item, 'local-before-mutation');
        }
        $payload = $this->contentPayload($action, $payload);
        $remote = $this->remote->mutate($siteId, $resource, $remoteId, $action, $payload);
        if ($action === 'delete') {
            $item?->delete();
        } elseif ($action === 'trash' && $item) {
            $item->update(['status' => 'trash', 'synced_at' => now()]);
        } else {
    ...
    public function mutateComment(int $siteId, int $remoteId, string $action, array $payload = []): array
    {
        ...
        if ($action === 'delete') {
            ...
            $this->upsertRemote($siteId, 'comments', $result);
        }
    ...
    public function mutateTerm(int $siteId, string $taxonomy, ?int $remoteId, string $action, array $payload = []): array
    {
        ...
        if ($action === 'delete') {
            TaxonomyTerm::query()->where('site_id', $siteId)->where('taxonomy', $taxonomy)->where('remote_id', $remoteId)->delete();
        } elseif (is_array($result)) {
            $this->upsertRemote($siteId, $resource, $result, $taxonomy);
        }
    ...
    public function compare(ContentRevision $a, ContentRevision $b): array
    {
        ...
        $diff = [];
        foreach ($keys as $key) {
            if (($a->snapshot[$key] ?? null) !== ($b->snapshot[$key] ?? null)) {
                $diff[$key] = ['from' => $a->snapshot[$key] ?? null, 'to' => $b->snapshot[$key] ?? null];
            }
        }
    ...
    private function upsertRemote(int $siteId, string $resource, array $row, ?string $customTaxonomy = null): bool
    {
        ...
        if (in_array($resource, ['posts', 'pages'], true)) {
            ...
            if ($old && $old !== $hash) {
                $this->snapshot($model, 'wordpress-sync');
            }
        ...
        if ($resource === 'media') {
            ...
            $old = $model->remote_hash;
            $model->fill(['title' => $this->rendered($row['title'] ?? null), 'slug' => $row['slug'] ?? null, 'mime_type' => $row['mime_type'] ?? null, 'media_type' => $row['media_type'] ?? null, 'source_url' => $row['source_url'] ?? null, 'alt_text' => $row['alt_text'] ?? null, 'caption' => $this->rawOrRendered($row['caption'] ?? null), 'description' => $this->rawOrRendered($row['description'] ?? null), 'metadata' => $row['media_details'] ?? [], 'remote_hash' => $hash, 'remote_modified_at' => $this->date($row['modified_gmt'] ?? null), 'synced_at' => now(), 'processing_state' => 'ready'])->save();
    ...
    private function guardConflict(int $siteId, string $resource, Model $local, array $expected): void
    {
        ...
        $remoteHash = $this->hash($remote);
        $remoteModified = $this->date($remote['modified_gmt'] ?? $remote['modified'] ?? null);
        $remoteVersion = (string) ($remote['version'] ?? $remote['modified_gmt'] ?? '');
        $expectedHash = $expected['hash'] ?? $local->remote_hash;
        $expectedModified = $this->date($expected['modified_at'] ?? $local->remote_modified_at);
        $expectedVersion = $expected['version'] ?? $local->remote_version;
    ...
    {
        if (in_array($action, ['publish', 'draft', 'pending'], true)) {
            $payload['status'] = $action === 'publish' ? 'publish' : $action;
        }
        if ($action === 'schedule') {
