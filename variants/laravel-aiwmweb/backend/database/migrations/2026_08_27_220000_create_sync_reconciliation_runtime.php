...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('content_conflicts', function (Blueprint $table) {
            $table->string('resource', 64)->nullable();
            $table->string('local_hash', 64)->nullable();
    ...
    public function down(): void
    {
        ...
        Schema::table('sync_runs', function (Blueprint $table) {
            $table->dropIndex('sync_runs_site_state_idx');
            $table->dropIndex('sync_runs_site_created_idx');
            $table->dropConstrainedForeignId('initiated_by_user_id');
            $table->dropConstrainedForeignId('resume_of_sync_run_id');
            $table->dropColumn([
                'mode', 'trigger', 'state', 'resources', 'metadata', 'lease_token',
                'discovered_count', 'created_count', 'updated_count', 'unchanged_count',
                'conflicted_count', 'deleted_count', 'failed_count', 'last_error',
            ]);
        });
    }
};
