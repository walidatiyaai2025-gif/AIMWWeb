...
public static class SecretProtectionStorage
{
    ...
    public static RuntimeLockLease AcquireWebRuntimeLease(string? localApplicationData = null)
    {
        var path = EnsureRuntimeLockFile(localApplicationData);
        try
        {
        ...
        catch (IOException ex)
        {
            throw new InvalidOperationException(
                "Offline recovery currently holds the exclusive AI WordPress Manager recovery lock. Wait for recovery to finish before starting the web application.",
                ex);
        }
    }

    public static RuntimeLockLease AcquireRecoveryExclusiveLease(string? localApplicationData = null)
    {
        var path = EnsureRuntimeLockFile(localApplicationData);
        try
        {
            ...
            writer.Write($"recoveryPid={Environment.ProcessId};startedUtc={DateTime.UtcNow:O}");
            writer.Flush();
        ...
        {
            throw new InvalidOperationException(
                "The AI WordPress Manager runtime lock is held by one or more web application workers. Stop every web application worker before offline recovery.",
                ex);
        }
    }

    [Obsolete("Use AcquireRecoveryExclusiveLease for offline recovery or AcquireWebRuntimeLease for the web host.")]
    public static RuntimeLockLease AcquireRuntimeLock(string? localApplicationData = null) =>
        AcquireRecoveryExclusiveLease(localApplicationData);
    ...
    private static string EnsureRuntimeLockFile(string? localApplicationData)
    {
        ...
        if (!File.Exists(path))
        {
            try
            {
                ...
                using var writer = new StreamWriter(stream, leaveOpen: true);
                writer.Write("AI WordPress Manager runtime/recovery coordination lock");
                writer.Flush();
                stream.Flush(flushToDisk: true);
                HardenFilePermissions(path);
            }
            ...
            {
                // Another process won the create race. The shared lock file is ready.
            }
        }
        return path;
    }
