...
public sealed class Site : SoftDeletableEntity, IAggregateRoot
{
    ...
    public Site(string name, Uri siteUrl, DateTime utcNow, Guid? ownerUserId = null)
    {
        SetName(name, utcNow);
    ...
    public void AssignOwner(Guid ownerUserId, DateTime utcNow)
    {
    ...
    public void SetSiteUrl(Uri siteUrl, DateTime utcNow)
    {
        ...
        if (siteUrl.Scheme is not ("http" or "https")) throw new ArgumentException("Site URL must use HTTP or HTTPS.", nameof(siteUrl));
        SiteUrl = siteUrl.GetLeftPart(UriPartial.Authority).TrimEnd('/');
        MarkUpdated(utcNow);
    }

    public void UpdateDiscovery(string? homeUrl, string? wordPressVersion, string? languageCode, DateTime utcNow)
    {
