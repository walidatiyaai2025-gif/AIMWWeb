    ...
    ISecretProtectionService secretProtectionService)
{
    ...
    public async Task<SiteMailProfileView> GetAsync(Guid siteId, CancellationToken cancellationToken = default)
    {
        ...
        return profile is null
            ? SiteMailProfileView.Default(siteId)
            : ToView(profile);
    }

    public async Task<SiteMailProfileView> SaveAsync(
        Guid siteId,
        SiteMailProfileInput input,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(input);
        await RequireOwnedSiteAsync(siteId, cancellationToken);

        var profile = await dbContext.SiteMailProfiles
            .FirstOrDefaultAsync(x => x.SiteId == siteId && x.OwnerUserId == OwnerId, cancellationToken);
        ...
        else
        {
            ...
            if (!string.IsNullOrWhiteSpace(input.ReplyToAddress))
                ValidateEmail(input.ReplyToAddress, "Reply-to address");

            profile.ConfigureSmtp(
                input.Host,
                input.Port,
                input.UserName,
                input.FromAddress,
        ...
        {
            if (input.Password.Length > 2048)
                throw new InvalidOperationException("SMTP password is too long.");
            var protectedPassword = await secretProtectionService.ProtectAsync(input.Password, cancellationToken);
            profile.SetProtectedPassword(protectedPassword, now);
        }
    ...
    public async Task<SiteMailDeliveryProfile?> GetDeliveryProfileAsync(Guid siteId, CancellationToken cancellationToken = default)
    {
        ...
        if (profile is null || !profile.IsEnabled || profile.UseAccountProfile)
            return null;
    ...
    {
        var ownsSite = await dbContext.Sites.AsNoTracking()
    ...
    private static SiteMailProfileView ToView(SiteMailProfile profile) => new(
        profile.SiteId,
        profile.UseAccountProfile,
        profile.Host,
        profile.Port,
        profile.UserName,
        !string.IsNullOrWhiteSpace(profile.ProtectedPassword),
        profile.FromAddress,
        profile.FromName,
        profile.ReplyToAddress,
        profile.EnableSsl,
...
public sealed record SiteMailProfileInput(
    bool UseAccountProfile,
    string Host,
    int Port,
    string UserName,
    string Password,
...
public sealed record SiteMailProfileView(
    Guid SiteId,
    bool UseAccountProfile,
    string Host,
    int Port,
...
{
    public static SiteMailProfileView Default(Guid siteId) => new(
        siteId, true, string.Empty, 587, string.Empty, false,
        string.Empty, string.Empty, string.Empty, true, false, null);
}

public sealed record SiteMailDeliveryProfile(
    string Host,
    int Port,
    string UserName,
    string? Password,
