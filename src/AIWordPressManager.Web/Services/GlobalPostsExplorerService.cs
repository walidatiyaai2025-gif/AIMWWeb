...
public sealed class GlobalPostsExplorerService(AppDbContext dbContext)
{
    ...
    public async Task<GlobalPostsExplorerResult> SearchPostsAsync(Guid? siteId, string? status, string? query, int page, int pageSize, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<GlobalPagesExplorerResult> SearchPagesAsync(Guid? siteId, string? status, string? query, int page, int pageSize, CancellationToken cancellationToken = default)
    {
        page = Math.Max(1, page);
        pageSize = Math.Clamp(pageSize, 10, 100);
    ...
    private static IQueryable<AIWordPressManager.Domain.Entities.WordPressContentRecord> ApplyFilters(IQueryable<AIWordPressManager.Domain.Entities.WordPressContentRecord> source, Guid? siteId, string? status, string? query, bool includeContent)
    {
...
public sealed record GlobalPagesExplorerResult(IReadOnlyList<GlobalPageExplorerItem> Items, int Total, int Page, int PageSize, int TotalPages, GlobalPagesSummary Summary);
public sealed record GlobalPageExplorerItem(Guid SiteId, string SiteName, int WordPressId, string Title, string Slug, string Status, string Link, string RenderedExcerpt, DateTime? ModifiedAtUtc, DateTime LastSynchronizedAtUtc);
public sealed record GlobalPagesSummary(int Total, int Published, int Drafts, int Pending, int Private, int SiteCount, DateTime? LastSynchronizedAtUtc);
