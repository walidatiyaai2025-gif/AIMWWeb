...
function Write-Log([string]$Message, [string]$Level = "INFO") {
    Add-Content -LiteralPath $script:LogPath -Encoding UTF8 -Value ("{0} [{1}] {2}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Level, $Message)
}
...
function Invoke-Native {
    ...
    if (-not [string]::IsNullOrWhiteSpace($script:LastOutput)) {
        Add-Content -LiteralPath $script:LogPath -Encoding UTF8 -Value $script:LastOutput
    }
...
function Test-NativeSuccess([string]$FilePath, [string[]]$Arguments) {
...
function Read-RequiredValue([string]$Prompt, [string]$DefaultValue = "") {
    ...
    while ($true) {
        $display = $Prompt
        if (-not [string]::IsNullOrWhiteSpace($DefaultValue)) { $display = "$Prompt [$DefaultValue]" }
...
function Enable-GitSafeDirectoryIfRequired([string]$Path) {
    ...
    $existingValues = @($existing | ForEach-Object { $_.ToString().Trim() })
    if ($existingValues -notcontains $safePath) {
...
function Ensure-Repository([bool]$AllowClone) {
    ...
    elseif (-not (Test-Path -LiteralPath (Join-Path $path ".git"))) {
        ...
        if ($AllowClone -and $entries.Count -eq 0) {
            ...
            throw "The selected directory exists but is not a Git repository: $path"
        }
    }
...
function Protect-LocalChanges {
    ...
    Write-WarningMessage "Uncommitted local changes were detected."
...
function Protect-LocalCommits {
    ...
    if ($NonInteractive) {
        throw "Local commits would be replaced by update. Push them first or run interactively to create a backup branch."
    }
    if (-not (Read-YesNo "Create a local backup branch for these commits before updating?" $true)) {
        throw "Update cancelled to protect local commits."
    }
...
function Stop-DotNetProcessesWithConsent {
    ...
    foreach ($process in $processes) {
        Write-Host ("  PID {0} - {1}" -f $process.Id, $process.ProcessName) -ForegroundColor Yellow
    }
...
function Clean-BuildOutput([string]$Root) {
    if ($SkipClean) {
        Write-WarningMessage "Build cleanup skipped."
        return
    }
...
function Run-Application([string]$Root) {
    ...
    if ([string]::IsNullOrWhiteSpace($url)) {
        $url = Read-RequiredValue "Enter server URL (example: http://0.0.0.0:7148)"
    }
    ...
    if ($Configuration -eq "Release") { $env:ASPNETCORE_ENVIRONMENT = "Production" }
    else { $env:ASPNETCORE_ENVIRONMENT = "Development" }
    ...
    Write-Host " Restore/build will run incrementally if required." -ForegroundColor DarkGray
    Write-Host " Press Ctrl+C to stop." -ForegroundColor Yellow
    Write-Host "====================================================" -ForegroundColor DarkCyan

    Invoke-Native "dotnet.exe" @("run", "--project", $project, "--configuration", $Configuration, "--no-launch-profile", "--urls", $url) "Application stopped with an error."
}
...
function Push-ToGitHub {
    ...
    if ($current -ne $Branch) {
        throw "Current branch is '$current'. The setup tool only pushes the configured '$Branch' branch."
    }

    $changes = @(Get-WorkingTreeChanges)
    if ($changes.Count -gt 0) {
        Write-WarningMessage "Uncommitted files exist. They cannot be pushed until they are committed."
    }
    ...
    [void][int]::TryParse($aheadText, [ref]$ahead)
    if ($ahead -eq 0) {
        Write-Success "Nothing to push."
        return
    }

    Write-Host "$ahead committed change(s) are ready to push." -ForegroundColor Yellow
    if (-not (Read-YesNo "Push committed changes to origin/$Branch?" $false)) {
...
function Diagnose {
    ...
    if (Get-Command dotnet.exe -ErrorAction SilentlyContinue) {
        ...
        if ($has8) { Write-Host "NET 8 SDK    : FOUND" -ForegroundColor Green }
            ...
            Write-Host "NET 8 SDK    : MISSING" -ForegroundColor Red
            $issues.Add(".NET 8 SDK is missing.")
        }
    }
    else {
        Write-Host "dotnet       : MISSING" -ForegroundColor Red
        $issues.Add("dotnet.exe is missing.")
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($InstallPath)) {
        ...
        if (-not (Test-Path -LiteralPath $path)) {
            $issues.Add("Repository directory does not exist.")
        }
        elseif (-not (Test-Path -LiteralPath (Join-Path $path ".git"))) {
            $issues.Add("Selected directory is not a Git repository.")
        }
        ...
        Write-Host "Repository   : not supplied; pass -InstallPath for repository checks" -ForegroundColor Yellow
    }

    if ($issues.Count -eq 0) {
        Write-Success "No blocking setup problems detected."
    }
    else {
...
try {
    ...
    if ($Mode -eq "Interactive") {
        ...
        $Mode = Show-Menu
        if ($Mode -eq "Exit") {
            Write-Success "No changes were made."
            exit 0
        }
    }
