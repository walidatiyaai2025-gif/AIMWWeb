...
public sealed class OfflineSecretRecoveryInstallerTests : IDisposable
{
    ...
    [Fact]
    public void InstallFromBackup_WebRuntimeLeaseBlocksKeyMutationBeforeEnvelopeDecryption()
    {
        ...
        {
            var backup = CreateRecoveryBackup(masterKey);
            using var runtimeLease = SecretProtectionStorage.AcquireWebRuntimeLease(localApplicationData);
            var installer = new OfflineSecretRecoveryInstaller();
            ...
            action.Should().Throw<InvalidOperationException>()
                .WithMessage("*runtime lock*Stop every web application worker*");
    ...
    [Fact]
    public void InstallFromBackup_RejectsDuplicateManifest()
    {
    ...
    private string CreateRecoveryBackup(byte[] masterKey)
    {
        ...
        WriteManifestEntry(archive, "manifest.json", 5, SecretRecoveryKeyEnvelopeFormat.WrappedKeyV1Mode, envelope);
        return path;
    }

    private static void WriteManifestEntry(ZipArchive archive, string name, int version, string mode, string envelope)
    {
