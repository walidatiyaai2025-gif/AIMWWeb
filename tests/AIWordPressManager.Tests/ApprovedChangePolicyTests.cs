...
public sealed class ApprovedChangePolicyTests
{
    ...
    {
        var item = CreateApproval(
            CreateRequest(DateTimeOffset.UtcNow),
            operationType: "AI.ContentUpdate");
    ...
    {
        var request = CreateRequest(DateTimeOffset.UtcNow) with { ForceOverwrite = true };
        var item = CreateApproval(request);
    ...
    public void Remote_matches_detects_idempotent_replay_without_second_post()
    {
        ...
        var remote = new WordPressEditableContent(
            ...
            null, modified, 9, [1, 2], [3], "", "open", "open", "standard", false);
