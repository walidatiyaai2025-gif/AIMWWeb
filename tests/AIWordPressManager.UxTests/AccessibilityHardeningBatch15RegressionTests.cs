...
public sealed class AccessibilityHardeningBatch15RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_15(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch15.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 15");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_15(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch15.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 15");
    }
}
