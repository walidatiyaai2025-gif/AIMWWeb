...
public sealed class AccessibilityHardeningBatch7RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_7(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardeningBatch7.IssuesAsync(page);
        issues.Should().BeEmpty($"public route {route.Path} must pass UX-010 accessibility hardening batch 7");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_7(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardeningBatch7.IssuesAsync(page);
        issues.Should().BeEmpty($"authenticated route {route.Path} must pass UX-010 accessibility hardening batch 7");
    }
}
