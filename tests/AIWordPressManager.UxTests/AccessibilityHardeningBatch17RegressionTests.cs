...
public sealed class AccessibilityHardeningBatch17RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_17(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch17.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 17");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_17(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch17.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 17");
    }
}
