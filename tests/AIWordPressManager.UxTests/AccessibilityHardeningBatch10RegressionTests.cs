...
public sealed class AccessibilityHardeningBatch10RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_10(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch10.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 10");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_10(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch10.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 10");
    }
}
