...
public sealed class AccessibilityHardeningBatch12RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_12(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch12.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 12");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_12(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch12.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 12");
    }
}
