...
public sealed class AccessibilityHardeningBatch13RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_13(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch13.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 13");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_13(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch13.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 13");
    }
}
