...
public sealed class AccessibilityHardeningBatch14RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_14(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch14.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 14");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_14(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch14.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 14");
    }
}
