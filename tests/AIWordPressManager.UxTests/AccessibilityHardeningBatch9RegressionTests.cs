...
public sealed class AccessibilityHardeningBatch9RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_9(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch9.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 9");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_9(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch9.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 9");
    }
}
