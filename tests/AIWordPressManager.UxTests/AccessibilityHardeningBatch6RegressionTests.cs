...
public sealed class AccessibilityHardeningBatch6RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_6(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardeningBatch6.IssuesAsync(page);
        issues.Should().BeEmpty($"public route {route.Path} must pass UX-010 accessibility hardening batch 6");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_6(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardeningBatch6.IssuesAsync(page);
        issues.Should().BeEmpty($"authenticated route {route.Path} must pass UX-010 accessibility hardening batch 6");
    }
}
