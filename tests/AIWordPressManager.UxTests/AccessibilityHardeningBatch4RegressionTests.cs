...
public sealed class AccessibilityHardeningBatch4RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_4(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardeningBatch4.IssuesAsync(page);
        issues.Should().BeEmpty($"public route {route.Path} must pass UX-010 accessibility hardening batch 4");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_4(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardeningBatch4.IssuesAsync(page);
        issues.Should().BeEmpty($"authenticated route {route.Path} must pass UX-010 accessibility hardening batch 4");
    }
}
