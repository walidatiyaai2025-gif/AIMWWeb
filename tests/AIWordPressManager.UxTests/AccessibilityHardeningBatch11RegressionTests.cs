...
public sealed class AccessibilityHardeningBatch11RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_11(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch11.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 11");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_11(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch11.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 11");
    }
}
