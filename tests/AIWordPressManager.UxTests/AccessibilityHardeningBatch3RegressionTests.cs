...
public sealed class AccessibilityHardeningBatch3RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_3(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardeningBatch3.IssuesAsync(page);
        issues.Should().BeEmpty($"public route {route.Path} must pass UX-010 accessibility hardening batch 3");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_3(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardeningBatch3.IssuesAsync(page);
        issues.Should().BeEmpty($"authenticated route {route.Path} must pass UX-010 accessibility hardening batch 3");
    }
}
