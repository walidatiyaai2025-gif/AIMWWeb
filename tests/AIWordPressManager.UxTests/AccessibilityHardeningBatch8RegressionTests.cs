...
public sealed class AccessibilityHardeningBatch8RegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_accessibility_hardening_batch_8(UxRouteCase route)
    {
        ...
        (await UxAccessibilityHardeningBatch8.IssuesAsync(page)).Should().BeEmpty($"public route {route.Path} must pass batch 8");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_accessibility_hardening_batch_8(UxRouteCase route)
    {
        ...
        await UxAudit.PrepareAsync(page);
        (await UxAccessibilityHardeningBatch8.IssuesAsync(page)).Should().BeEmpty($"authenticated route {route.Path} must pass batch 8");
    }
}
