...
public sealed class AccessibilityHardeningRegressionTests(UxTestHost host)
{
    ...
    public async Task Public_routes_pass_extended_aria_semantics(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardening.IssuesAsync(page);
        issues.Should().BeEmpty($"public route {route.Path} must preserve extended ARIA/accessibility semantics");
    }

    [Theory]
    [MemberData(nameof(AuthenticatedRouteCases))]
    public async Task Authenticated_routes_pass_extended_aria_semantics(UxRouteCase route)
    {
        ...
        var issues = await UxAccessibilityHardening.IssuesAsync(page);
        issues.Should().BeEmpty($"authenticated route {route.Path} must preserve extended ARIA/accessibility semantics");
    }
}
